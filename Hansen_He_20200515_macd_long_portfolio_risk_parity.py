...
def initialize(context):
    ...
    context.tickers = [context.DBC, context.SPY, context.IEI,\
                       context.GLD, context.IEF, context.TLT]
    
    context.start_month = get_datetime().month
    context.weights = {}
    ...
    schedule_function(separate, date_rules.every_day(),\
                      time_rules.market_close(minutes=10))
    schedule_function(close_position, date_rules.every_day(),\
                      time_rules.market_close(minutes=10))
    schedule_function(my_record_vars, date_rules.every_day(),\
                      time_rules.market_close(minutes=10))
    schedule_function(open_position, date_rules.every_day(),\
                      time_rules.market_close(minutes=10))
    schedule_function(empty_position, date_rules.every_day(),\
                      time_rules.market_close(minutes=10))
    # schedule_function(leverage_check, date_rules.every_day(), time_rules.market_close(minutes=5))
    schedule_function(daily, date_rules.every_day(),\
...
# # Will be called everyday for the securities you specify. 
# def leverage_check(context, data):
...
#         for ticker in context.tickers:
#             # Liquidate the short position if the leverage is approaching the limit

#                 # Need to liquidate short position
#                 if context.sold[ticker] == True and\
#                 context.portfolio.positions[ticker].amount < 0:

#                     log.info("Liquidating position %s" % (ticker))
#                     order_target_percent(ticker, 0)
#                     context.sold[ticker] = False

def rebalance(context, data):
    separate(context,data)
    log.info('Rebalancing; Backtest start month: %s; current month: %s'% (context.start_month, get_datetime().month))
    # check if it is January
    if get_datetime().month != context.start_month:
        return
    ...
    omega = np.matrix(cov_mat.values)  # omege matrix

    a, b = np.linalg.eig(np.array(cov_mat)) 
    a = np.matrix(a)
    b = np.matrix(b)
...
def open_position(context,data):
    for ticker in context.tickers:
        ...
        if macd[-1] > signal[-1] and not context.bought[ticker]:
            ...
            log.info("Opened Long %s Position; MACD_val: %.5f; Price: %.5f; Bought: %s; Sold: %s" %\
...
def my_record_vars(context, data):
    for ticker in context.tickers:
        ...
        if context.bought[ticker] or context.sold[ticker]:
            log.info("Holding %s position; MACD_val: %.5f; Bought: %s; Sold: %s" %\
                     (ticker, macd[-1]-signal[-1],\
                      context.bought[ticker], context.sold[ticker]))
      
def empty_position(context, data):
    for ticker in context.tickers:
        macd,signal = _get_macd(data, ticker)
        
        if context.portfolio.positions[ticker].amount == 0 and not context.bought[ticker] and not context.sold[ticker]:
            log.info("Empty %s position; MACD_val: %.5f; Bought: %s; Sold: %s" %\
                     (ticker, macd[-1]-signal[-1],\
                      context.bought[ticker], context.sold[ticker]))
...
def close_position(context, data): 
    for ticker in context.tickers:
        ...
        if macd[-1] < signal[-1] and\
        context.portfolio.positions[ticker].amount != 0 and\
        context.bought[ticker]:
            order_target_percent(ticker, 0)
            
            context.bought[ticker] = False
           
            log.info("Closed Long %s Position; MACD_val: %.5f; Price: %.5f; Bought: %s; Sold: %s" %\
                     (ticker, macd[-1]-signal[-1],\
                      data.current(ticker,'price'), context.bought[ticker],\
                      context.sold[ticker]))
